...
function MobileNavButton({ item, active }: { item: { href: string; label: string }; active: boolean }) {
  ...
  if (image) {
    return <Link href={item.href} aria-label={item.label} aria-current={active ? "page" : undefined} className="transition-transform hover:scale-105 active:scale-100"><img src={image || "/placeholder.svg"} alt="" className={`h-14 w-14 object-contain drop-shadow-[0_1px_3px_rgba(0,0,0,0.7)] ${active ? "brightness-125" : "brightness-95"}`} /></Link>
  }
...
export function Navigation() {
  ...
  return <>
    ...
    <div className="md:hidden fixed bottom-0 left-0 right-0 z-50"><div className="crest-menu-container absolute -top-8 left-1/2 -translate-x-1/2 z-20"><button onClick={(e) => { e.stopPropagation(); setIsCrestMenuOpen(!isCrestMenuOpen) }} aria-label="Open menu" className={`relative w-16 h-16 rounded-full border-2 border-yellow-600 shadow-lg shadow-black/50 flex items-center justify-center bg-[#1a0808] transition-opacity duration-200 ${isCrestMenuOpen ? "opacity-0 pointer-events-none" : "opacity-100"}`}><img src="/images/icons/avatar-frame.jpeg" alt="" className="absolute inset-0 w-16 h-16 object-contain" /><img src={getCrestImage(currentAppTheme)} alt="Menu" className="w-11 h-11 object-contain relative z-10" /></button>{isCrestMenuOpen && <div className="absolute bottom-full mb-3 left-1/2 -translate-x-1/2 w-64"><ArchiveFrame className="w-64 animate-archive-unfurl"><div className="p-1.5 pt-9">{crestMenuItems.map((item, index) => <Link key={item.href} href={item.href} className={`flex items-center gap-4 px-4 py-3.5 rounded-lg hover:bg-accent-gold/10 transition-colors ${index !== crestMenuItems.length - 1 ? "border-b border-accent-gold/10" : ""}`} onClick={() => setIsCrestMenuOpen(false)}><item.icon className="w-5 h-5 text-accent-gold" /><div className="flex-1"><span className="text-foreground font-cinzel text-sm uppercase tracking-wide">{item.label}</span></div>{item.badge && item.badge > 0 && <span className="bg-red-500 text-white text-[10px] rounded-full h-5 min-w-[20px] flex items-center justify-center px-1.5">{item.badge > 99 ? "99+" : item.badge}</span>}</Link>)}{user && <button className="flex items-center gap-4 px-4 py-3.5 rounded-lg hover:bg-accent-gold/10 transition-colors w-full text-left border-t border-accent-gold/20" onClick={handleLogout}><LogOut className="w-5 h-5 text-accent-gold" /><span className="text-foreground font-cinzel text-sm uppercase tracking-wide">{t("nav.logout")}</span></button>}</div></ArchiveFrame><button onClick={(e) => { e.stopPropagation(); setIsCrestMenuOpen(false) }} aria-label="Close menu" style={{ "--crest-rise": "150px" } as Record<string, string>} className="animate-archive-crest-rise absolute -top-7 left-1/2 -translate-x-1/2 z-30 w-16 h-16 rounded-full border-2 border-yellow-600 shadow-lg shadow-black/50 flex items-center justify-center bg-[#1a0808]"><img src="/images/icons/avatar-frame.jpeg" alt="" className="absolute inset-0 w-16 h-16 object-contain" /><img src={getCrestImage(currentAppTheme)} alt="Close menu" className="w-11 h-11 object-contain relative z-10" /></button></div>}</div><div className="flex items-center justify-center gap-3 px-3 pt-2 pb-2 bg-gradient-to-t from-black/80 via-black/40 to-transparent">{mobileNavItems.slice(0, 2).map(item => <MobileNavButton key={item.href} item={item} active={isActive(item.href)} />)}<div className="w-16" />{mobileNavItems.slice(2).map(item => <MobileNavButton key={item.href} item={item} active={isActive(item.href)} />)}</div></div>
  </>
}
