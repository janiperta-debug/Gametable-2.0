  ...
  return (
    <main data-theme={data.id as AppThemeName} className={`artifact-cabinet min-h-screen px-3 py-5 sm:px-6 sm:py-8 ${data.id === "conservatory" ? conservatoryStyles.conservatoryMaterials : ""} ${data.id === "spa" ? spaContrastStyles.spaContrast : ""}`} style={{ backgroundColor: "hsl(var(--background))" }}>
      <div className="mx-auto max-w-5xl space-y-4">
        ...
        <ArchiveFrame weight="thin" cornerSize="sm" className="rounded-md"><div className="flex flex-wrap items-center gap-3 p-4 sm:gap-4"><span className={`flex h-10 w-10 flex-none items-center justify-center rounded-full border border-[var(--archive-gold,#d9b65c)]/50 ${goldText}`} aria-hidden="true">{isActive || isUnlocked ? <Check className="h-5 w-5" /> : <Lock className="h-5 w-5" />}</span><div className="min-w-0 flex-1"><p className={`font-cinzel text-sm font-bold uppercase tracking-wide ${goldText}`}>{isActive ? t("themes.roomPage.themeActive") : isUnlocked ? t("themes.roomPage.themeUnlocked") : t("themes.roomPage.themeLocked")}</p><p className="font-body text-sm leading-snug text-[var(--archive-ink-soft)] text-pretty">{isActive ? t("themes.roomPage.themeActiveDesc") : isUnlocked ? t("themes.roomPage.themeUnlockedDesc") : t("themes.roomPage.themeLockedDesc")}</p></div><span className={`flex-none rounded-md border border-[var(--archive-gold,#d9b65c)]/40 px-2.5 py-1 font-cinzel text-xs font-bold uppercase tracking-wide ${goldText}`}>{`${t("themes.roomPage.levelLabel")} ${level}`}</span>{isActive ? <span className={`inline-flex min-h-9 flex-none items-center gap-1.5 rounded-md border border-[var(--archive-gold,#d9b65c)]/50 bg-[var(--archive-gold,#d9b65c)]/15 px-3 font-cinzel text-xs font-bold uppercase tracking-wide ${goldText}`}><Check className="h-4 w-4" aria-hidden="true" />{t("themes.roomPage.inUse")}</span> : isUnlocked ? <button type="button" onClick={() => setAppTheme(data.id as AppThemeName)} className={`inline-flex min-h-9 flex-none items-center rounded-md border border-[var(--archive-gold,#d9b65c)]/60 px-3 font-cinzel text-xs font-bold uppercase tracking-wide ${goldText} transition-colors hover:bg-[var(--archive-gold,#d9b65c)]/15`}>{t("themes.roomPage.useTheme")}</button> : null}</div></ArchiveFrame>
