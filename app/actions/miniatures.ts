...
export async function addMiniatureToCollection(
  ...
  if (!unit.catalogId) return { success: false, error: "Miniature is not a canonical catalog result" }
  ...
  const armyContext = army?.id ? army : undefined
  if (armyContext) {
    const { data: validatedArmy, error: armyError } = await supabase
      .from("mini_armies")
      .select("id, faction_id")
      .eq("id", armyContext.id)
      .eq("user_id", user.id)
      .maybeSingle()
    ...
    if (!xpResult.success) console.error("Miniature ownership was created but XP award failed:", xpResult.error)
    return { success: true }
  }
...
export interface MiniatureBulkImportLine { catalogId: string; modelCount: number }
export interface MiniatureBulkImportResult { success: boolean; error?: string; insertedCount?: number }
...
export async function importMiniaturesToCollection(
  ...
  if (!Array.isArray(lines) || lines.length === 0) return { success: false, error: "No Miniatures to import" }
  for (const line of lines) {
    if (!line.catalogId) return { success: false, error: "Every imported Miniature requires a canonical catalog match" }
    if (!Number.isInteger(line.modelCount) || line.modelCount < 1) return { success: false, error: "Miniature quantity must be a positive integer" }
  }
  ...
  let validatedArmy: { id: string; faction_id: string } | undefined
  if (army?.id) {
    const { data, error } = await supabase
      .from("mini_armies")
      .select("id, faction_id")
    ...
    if (!data) return { success: false, error: "Army not found" }
    validatedArmy = data
  }
  ...
  if (canonicalById.size !== catalogIds.length) return { success: false, error: "One or more imported Miniatures could not be resolved to a canonical catalog unit" }

  const payloads: Record<string, string | number | boolean | null>[] = []
  for (const line of lines) {
    const payload = buildMiniatureArmyUnitPayload({
      catalogId: line.catalogId,
      army: validatedArmy ? { id: validatedArmy.id, factionId: validatedArmy.faction_id } : undefined,
  ...
  const { data: insertedUnits, error } = await supabase.from("mini_army_units").insert(payloads).select("id")
  if (error) return { success: false, error: error.message }
  if (!insertedUnits?.length) return { success: false, error: "No Miniatures were imported" }

  const xpResult = await awardCategoryImportXP(user.id, "miniatures")
  if (!xpResult.applied) console.info("Miniatures category import XP was not applied", { userId: user.id, insertedCount: insertedUnits.length })
  return { success: true, insertedCount: insertedUnits.length }
}

export async function parseRosterFile(xmlContent: string): Promise<Array<{ name: string; quantity: number; faction?: string; points?: number }>> {
  const parsed: Array<{ name: string; quantity: number; faction?: string; points?: number }> = []
  try {
    const selectionRegex = /<selection[^>]*\sname="([^"]+)"[^>]*\snumber="(\d+)"[^>]*(?:\scosts="([^"]*)")?[^>]*>/gi
    let match
    while ((match = selectionRegex.exec(xmlContent)) !== null) {
      ...
      if (!name.includes("Upgrade") && !name.includes("Wargear")) parsed.push({ name, quantity, points })
    }
    const forceMatch = xmlContent.match(/<force[^>]*\scatalogueName="([^"]+)"[^>]*>/i)
    if (forceMatch) parsed.forEach((item) => { item.faction = forceMatch[1] })
  } catch (error) { console.error("Error parsing roster file:", error) }
  return parsed
}

export async function parseArmyList(text: string): Promise<Array<{ name: string; quantity: number; points?: number }>> {
  ...
  for (const line of text.split("\n").filter((line) => line.trim())) {
    ...
    if (match) parsed.push({ quantity: parseInt(match[1], 10), name: match[2].trim(), points: match[3] ? parseInt(match[3], 10) : undefined })
    else if (reverseMatch) parsed.push({ name: reverseMatch[1].trim(), quantity: parseInt(reverseMatch[2], 10), points: reverseMatch[3] ? parseInt(reverseMatch[3], 10) : undefined })
  }
  return parsed
}
...
export async function getUserMiniatureCollection(system?: MiniatureSystem) {
  ...
  try {
    const { data, error } = await supabase.from("mini_army_units").select(`
      id, model_count, points_total, paint_status, custom_name, upgrades, is_warlord, owned,
      unit:mini_units ( id, name, unit_type, base_points, model_count_min, model_count_max,
        faction:mini_factions ( id, name, system:mini_systems ( id, code, name ) ) )
    `).eq("user_id", user.id).eq("owned", true)
    if (error) return { success: false, error: error.message, data: [] }
    let filtered = data || []
    if (system) filtered = filtered.filter((entry: any) => entry.unit?.faction?.system?.code === system)
